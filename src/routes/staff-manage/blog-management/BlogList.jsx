...
import { PlusCircle, Eye, Filter, Search, Edit } from "lucide-react";
...
const BlogList = () => {
  ...
  // Fetch blogs based on statusFilter
  useEffect(() => {
  ...
  const getEmptyStateMessage = () => {
    ...
    return searchTerm
      ? `Không tìm thấy blog nào với trạng thái "${statusFilter}" khớp với tìm kiếm`
      : `Không có blog nào với trạng thái "${statusFilter}"`;
  };
                ...
                <tbody>
                  {displayedBlogs.map((blog, index) => (
                    <tr key={blog.id} className="border-b hover:bg-gray-400 transition-colors">
                      ...
                      <td className="px-3 py-4 text-center">
                        {blog.images && <img src={blog.images} alt={blog.blogTitle} className="w-16 h-16 object-cover rounded-lg" />}
                      </td>
                      <td className="px-3 py-4 text-center">
                        <Link to={`/manage/blog/${blog.blogId}`} className="hover:text-neutral-300">
                          {blog.blogTitle}
                        </Link>
                      </td>
                      ...
                      <td className="px-3 py-4 text-center">
                        <span
                          className={`inline-flex items-center px-2 py-0.5 rounded-full font-normal text-xs md:text-sm ${blog.status === "Published"
                      ...
                      <td className="px-3 py-4 flex justify-center gap-2">
                        <Link
                          ...
                          className="bg-yellow-100 text-yellow-700 hover:bg-yellow-400 p-1.5 md:p-2 rounded-lg transition-colors cursor-pointer mt-4"
                        >
                ...
                >
                  <div className="flex flex-col gap-2">
                    <div className="flex justify-between">
                      ...
                      <span
                        className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-normal ${blog.status === "Published"
                          ? "bg-green-100 text-green-800"
                          : blog.status === "Pending"
                            ? "bg-yellow-100 text-yellow-800"
                            : "bg-red-100 text-red-800"
                    ...
                    <p className="text-sm">
                      <span className="font-medium">Tiêu đề:</span> {blog.title}
                    </p>
                    <p className="text-sm">
                    ...
                    <Link
                      ...
                      data-tooltip-id="action-tooltip"
                      data-tooltip-content="Cập nhật"
                      className="bg-yellow-100 text-yellow-700 hover:bg-yellow-400 p-2 rounded-lg flex items-center justify-center gap-2 text-sm mt-2"
                    >
                      <Edit className="w-4 h-4" />
                    </Link>
                  </div>
